...
def build_graph_from_items(items: List[Dict]):
    ...
    Nodes: item:<id>, entity:<name>
    Edges: item - HAS_ENTITY
    Also add SIMILAR_TO edges later (external).
    """
    G = nx.Graph()
    for it in items:
...
def add_similarity_edges(G, items, embeddings, id2idx, top_k=5):
    """
    For each item, find top_k nearest in embedding space and add SIMILAR_TO edges.
    """
    ...
    index.add(xb)
    D, I = index.search(xb, top_k+1)  # includes self
