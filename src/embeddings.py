...
# singleton embedder to avoid reloading model multiple times
_EMBEDDER = None
...
def build_embeddings(items: list, model_name=EMBED_MODEL, out_npy=EMBEDDINGS_NPY, map_path=EMBEDDING_MAP):
    """
    Items: list of dicts with at least 'item_id' and 'description' or 'text'.
    If embeddings file already exists, load it instead of recomputing.
    """
    out_npy.parent.mkdir(parents=True, exist_ok=True)

    # If embeddings already exist on disk, load and return
    ...
    # Otherwise compute embeddings and save
