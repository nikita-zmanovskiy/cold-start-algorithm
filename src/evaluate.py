...
def coverage_at_k(recommendations_all_users: dict, k=10):
    # recommendations_all_users: user_id -> list of recs (dicts with item_id)
    items = set()
    for user, recs in recommendations_all_users.items():
        for r in recs[:k]:
            items.add(str(r["item_id"]))
    # total items can be deduced from pool or passed; here return fraction over unique recommended items
    return len(items)
