...
def create_from_csv(path, out_path, rating_threshold=4.0, last_n=None, user_col='user_id', item_col='item_id', rating_col=None, time_col=None):
    ...
    # If last_n specified or timestamp exists -> use last-N interactions per user
    if last_n is not None or time_col is not None:
        # build mapping user -> list of (ts, item)
        d = defaultdict(list)
        for r in rows:
            uid = str(r[user_col])
            iid = str(r[item_col])
        ...
        for uid, recs in d.items():
            # sort by timestamp if available; if ts None then keep original order
            ...
            gt[uid] = list(dict.fromkeys(picked))  # uniq preserving order
    else:
        # fallback to rating threshold
        if rating_col is None:
